...
const Input = () => {
  ...
  const [uniquePostIds, setUniquePostIds] = useState(new Set());
  ...
  const { fetchedData, fetchedLoading, fetchedError } = useFetch(
  ...
  const handleSearch = async () => {
    try {
      ...
      setUniquePostIds((prevIds) => new Set([...prevIds, ...newPostIds]));
  ...
  const handleLoadMore = async () => {
    ...
    try {
      ...
      setUniquePostIds((prevIds) => {
        const newPostIds = data.data.map((post) => post.id);
        return new Set([...prevIds, ...newPostIds]);
      });

      setSearchResults((prevResults) => {
        const uniqueData = data.data.filter((newPost) => {
          return !uniquePostIds.has(newPost.id);
        });
  ...
  const handleInputChange = (e) => {
    ...
    // Устанавливаем очищенное значение в состояние
          ...
          <div className={cl.card}>
            {searchResults?.data?.map((result, index) => (
                <div key={`${result.id}-${index}`} className={cl.searchResult}>
                  ...
                  <Link to={`/previewPage/${result.id}?categoryId=${result?.attributes?.category?.data?.id}`}>
                    <img
